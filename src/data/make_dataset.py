...
def main(input_filepath, output_filepath):
    ...
    input_filepath= click.prompt('Enter the file path for the input data', type=click.Path(exists=True), default='./data/raw/admission.csv')
    output_folderpath = click.prompt('Enter the folder path for the output preprocessed data', type=click.Path(), default='./data/processed/')
...
def process_data(input_filepath, output_folderpath):
    ...
    #--Importing dataset
    df = pd.read_csv(input_filepath, sep=",", header=0, low_memory=False, encoding='utf-8')
    print(df.head())
    ...
    for file, filename in zip([X_train_scaled_df, X_test_scaled_df, y_train, y_test], ['X_train', 'X_test', 'y_train', 'y_test']):
